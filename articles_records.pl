...
$sth11r=$dbh->prepare("CREATE TABLE article_records(title varchar(500), 
authid varchar(200),
...
sub insert_article()
{
	...
	$sth1=$dbh->prepare("insert into article_records values('$title','$authids','$author_name','$featid','$page','$page_end','$plates','$volume','$part','$year','$month','$id')");
...
sub get_featid()
{
	...
	$sth=$dbh->prepare("select featid from feature_records where feat_name='$feature'");
	$sth->execute();
